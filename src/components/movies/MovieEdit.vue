              ...
              <el-row :gutter="20">
                ...
                <el-col :span="24">
                  <el-form-item label="Galerie d'images">
                    <el-upload name="images"
                               :action="`${apiRoot}/upload/images`"
                               :file-list="fileList"
  ...
  export default {
    ...
    created() {
      Services.getMovie(this.id).then(res => {
        ...
        res.data.imageSet.forEach(image => {
          this.fileList.push({'name': image, 'url': `${api.ftpUrl}/${image.id}`})
        });
    ...
    methods: {
      ...
      handleListSuccess(res, file) {
        this.finalImageSet = this.form.imageSet.filter(img => typeof img !== 'string')
        this.finalImageSet.push({name: res.original_filename, id: res.public_id})
        this.$notify({
      ...
      pushImages(file, fileList) {
        this.imageSet = fileList.map(file => file.name);
      },
      handleRemove(file, fileList) {
        const dataSet = fileList.filter((image => {
          return image.name.id !== file.name.id
        })).map(image => image.name);
        this.finalImageSet = dataSet
      },
      handlePictureCardPreview(file) {
        this.dialogImageUrl = `${api.ftpUrl}/${file.name.id}`;
      ...
      onSubmit(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if(this.finalImageSet.length) {
              this.form.imageSet = this.finalImageSet
            }
      ...
      deleteMovie() {
        this.$confirm('Etes vous sûr de vouloir supprimer ce film ?')
          .then(() => {
            Services.removeMovie(this.id).then(() => {
              this.$notify({
                title: 'Suppression',
